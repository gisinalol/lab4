...
int main(int argc, char **argv)
	...
	&final, 1, MPI_DOUBLE, NextProcNum, 2, MPI_COMM_WORLD, &status);
	fflush(stdout);
	if (myrank != 0)
	MPI_Sendrecv(&u[0], 1, MPI_DOUBLE, PrevProcNum, 2,
	&prev, 1, MPI_DOUBLE, PrevProcNum, 2, MPI_COMM_WORLD, &status);
	MPI_Gather((void *)u, m, MPI_DOUBLE, (void *)U, m, MPI_DOUBLE, 0, MPI_COMM_WORLD); //сбор информации
	if (!myrank && (j % 10 == 0))
		{
		for (int i = 0; i < n; i++)
		fprintf(file, "%d %.10f\n", i, U[i]);
	...
	if (!myrank)
		...
		if (!myrank)
		{
		FILE *fid = fopen("comm", "w+"); // открываем файл для записи формул gnuplot
		fprintf(fid, "set xrange[0:%d]\n", n - 1);
		fprintf(fid, "set yrange[-200:200]\n");
		fprintf(fid, "do for [i=0:%d]{\n", (int)(T_MAX / (10 * delta_t)));
		fprintf(fid, "plot 'gnuplot_par.plt' index i using 1:2 smooth bezier\npause 0.1}\npause -1\n");
		fclose(fid);
		}
	if (!myrank)
		fclose(file);
